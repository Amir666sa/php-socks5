...
👈🏻 تعرفه هر 1 سکه 100 تک تومان است
❗یعنی 10 عدد سکه = هزار تومان",
   'reply_to_message_id'=>$message_id,
   'reply_markup'=>json_encode([
        'keyboard'=>[
		[['text'=>'🔙 بازگشت']],
...
elseif($text == '📞ارسال اکانت📞'){
   ...
   'reply_markup'=>json_encode([
        'inline_keyboard'=>[
   [['text'=>'🔥پیوی ادمین🔥','url'=>'t.me/TexoSource']],
              ]
        ])
   ]);
$connect->query("UPDATE `user` SET `step` = 'none' WHERE `id` = '$from_id' LIMIT 1");
...
elseif($message){
	bot('sendmessage',[
	'chat_id'=>$from_id,
	'text'=>"🔥ربات $botname اولین ربات بمبر در کل تلگرام!!!",
    'reply_to_message_id'=>$message_id,
    'reply_markup'=>$home
    		]);
if($user['id'] != true)
